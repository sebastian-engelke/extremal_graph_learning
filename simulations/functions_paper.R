  ...
  for (k in 1:d) {
    ...
    if (reg_method == "glasso") {
      ...
      null.vote[-k, -k, ] <- null.vote[-k, -k, , drop = FALSE] +
        (abs(gl.tmp) <= thr_zero) ## make sure is consistent with default threshold value
...
sim_study <- function(d = 5,
  ...
  tbl <- tibble(
    ...
    bind_rows(tibble(
      type = paste0("F1_score", 1:length(rholist)),
      value = F1
    )) %>%
    bind_rows(tibble(
      type = c("incoh_pos_GLi", "incoh_pos_NSi"),
      value = incoh_pos
    )) %>%
    bind_rows(tibble(
      type = c("partial_pos", "partial_neg"),
    ...
    bind_rows(tibble(
      type = c("F1_ns_aic", "F1_ns_bic", "F1_ns_mbic"),
      value = F1_ic
    ))
...
GLNSi <- function(Sig, cor.scale = TRUE, pessimistic = FALSE) {
  ...
  NSi <- min(sapply(1:d, function(a) {
    if (sum(Th[-a, a] != 0) == 0) {
      ...
      Sc <- which(Th[, a] == 0)
      if (pessimistic) {
        return(incoherence_pess(Sig, S, Sc))
      } else {
        signs <- sign(Th[S, a])
...
generate_block_model <- function(ncliques, clique_size, alphad = 1) {
  ...
  for (i in 1:ncliques) {
    ...
    if (i == 1) {
      ...
      GG[(kk + (i - 2) * (kk - 1)):(kk + (i - 2) * (kk - 1) + kk - 1), (kk + (i - 2) * (kk - 1)):(kk + (i - 2) * (kk - 1) + kk - 1)] <- G1
    }
  }
  G <- complete_Gamma(GG)
...
create_palette_levels <- function(reg_method_n, palette_tbl) {
  ...
  my_tbl <- tibble(
    reg_method = purrr::map_chr(str_spl, function(el) {
      el[1]
    }),
    level = purrr::map_chr(str_spl, function(el) {
      el[2]
    })
  ) %>%
